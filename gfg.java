...
class GFG {
  ...
  static int getHeight(int in[], int level[], int start, int end, int n) {
    ...
    if (leftCount > 0)
      lheight = getHeight(in, newLeftLevel, start, getIndex - 1, leftCount);

    // Recursively call to calculate
    // height of right Subtree
    if (rightCount > 0)
      rheight = getHeight(in, newRightLevel, getIndex + 1, end, rightCount);
  ...
  public static void main(String[] args) {
    ...
    String inTraversal = inp.next();
    String levelTraversal = inp.next();
    String i2 = inTraversal.substring(1, inTraversal.length() - 1);
    String l2 = levelTraversal.substring(1, levelTraversal.length() - 1);
    int iarr[] = System.out.print("--------------" + i2 + "-------" + l2);
